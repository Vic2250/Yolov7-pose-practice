...
def color_list():
    ...
    return [hex2rgb(h) for h in
            matplotlib.colors.TABLEAU_COLORS.values()]  # or BASE_ (8), CSS4_ (148), XKCD_ (949)


def hist2d(x, y, n=100):
    # 2d histogram used in labels.png and evolve.png
    xedges, yedges = np.linspace(x.min(), x.max(), n), np.linspace(y.min(),
                                                                   y.max(), n)
...
def plot_one_box_PIL(box, img, color=None, label=None, line_thickness=None):
    ...
    if label:
        ...
        draw.rectangle(
            ...
            fill=tuple(color))
        draw.text((box[0], box[1] - txt_height + 1), label,
    ...
    for i, img in enumerate(images):
        ...
        if paths:
            ...
            cv2.putText(mosaic, label, (block_x + 5, block_y + t_size[1] + 5),
                        0, tl / 3, [220, 220, 220], thickness=tf,
...
def plot_study_txt(path='',
    ...
    for f in sorted(Path(path).glob('study*.txt')):
        ...
        ax2.plot(y[6, 1:j], y[3, 1:j] * 1E2, '.-', linewidth=2, markersize=8,
                 label=f.stem.replace('study_coco_', '').replace('yolo',
                                                                 'YOLO'))
...
def plot_evolution(
        yaml_file='data/hyp.finetune.yaml'):  # from utils.plots import *; plot_evolution()
    ...
    for i, (k, v) in enumerate(hyp.items()):
        ...
        plt.subplot(6, 5, i + 1)
        plt.scatter(y, f, c=hist2d(y, f, 20), cmap='viridis', alpha=.8,
                    edgecolors='none')
            ...
            for i, a in enumerate(ax):
                if i < len(results):
                    label = labels[fi] if len(labels) else f.stem.replace(
                        'frames_', '')
                    a.plot(t, results[i], marker='.', label=label, linewidth=1,
                           markersize=5)
                    a.set_title(s[i])
                    a.set_xlabel('time (s)')
                    # if fi == len(files) - 1:
...
def plot_results_overlay(start=0,
                         stop=0):  # from utils.plots import *; plot_results_overlay()
    # Plot training 'results*.txt', overlaying train and val losses
    s = ['train', 'train', 'train', 'Precision', 'mAP@0.5', 'val', 'val', 'val',
         'Recall', 'mAP@0.5:0.95']  # legends
...
def plot_results(start=0, stop=0, bucket='', id=(), labels=(), save_dir=''):
    ...
    assert len(
        files), 'No results.txt files found in %s, nothing to plot.' % os.path.abspath(
        save_dir)
    for fi, f in enumerate(files):
        try:
            results = np.loadtxt(f, usecols=[2, 3, 4, 8, 9, 12, 13, 14, 10, 11],
...
# 將 EasyOCR 模型移至 GPU 上進行運算
reader = easyocr.Reader(['en'], gpu=True, model_storage_directory='EasyOCR')


def word_identify(im, bbox, last_x, last_y, last_confidence, last_text):
    # Draw bounding box
    x_center, y_center, width, height = bbox
    x1 = max(0, int(x_center - (width / 2)))
    y1 = max(0, int(y_center - (height / 2)))
    x2 = min(im.shape[1] - 1, int(x_center + (width / 2)))
    y2 = min(im.shape[0] - 1, int(y_center + (height / 2)))

    if x2 - x1 < 100 or y2 - y1 < 100:
        return '', last_x, last_y, 0
    
    # 將 ROI 轉換為 NumPy 陣列準備進行 EasyOCR 辨識
    roi_np = np.array(im[y1:y2, x1:x2])
    # 使用 GPU 進行 EasyOCR 辨識，將 roi_np 作為輸入
    result = reader.readtext(roi_np, detail=True)
    filtered_result = ''
    confidence = 0

    for res in result:
        if res[2] > 0.7:
            filtered_result = res[1]
            confidence = res[2]
        
    if len(filtered_result) < 6:
        filtered_result, confidence = 'Unknown', 0
    if last_x is not None and last_y is not None:
        if abs(last_x - x_center) < 100 and abs(last_y - y_center) < 100:
            if last_text == 'Unknown':
                pass
            else:
                if confidence < last_confidence:
                    filtered_result = last_text
                    confidence = last_confidence
                else:
                    pass
    return filtered_result, x_center, y_center, confidence


def plot_skeleton_kpts(im, bbox, kpts, steps, orig_shape=None):
    ...
    for sk_id, sk in enumerate(skeleton):
        ...
        pos1 = (
        int(kpts[(sk[0] - 1) * steps]), int(kpts[(sk[0] - 1) * steps + 1]))
        pos2 = (
        int(kpts[(sk[1] - 1) * steps]), int(kpts[(sk[1] - 1) * steps + 1]))
        if steps == 3:
